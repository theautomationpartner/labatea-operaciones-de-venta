 ...
 *
 * Existe para responder una sola pregunta que desde afuera no se puede contestar: cómo quedan los
 * archivos de `api/` una vez desplegados —¿siguen siendo `.ts` o Vercel los compila a `.js`?—,
 * porque de eso depende con qué extensión hay que escribir los imports relativos.
 *
...
export default async function handler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    info.dirError = (e as Error).message
  }
  ...
  /* La prueba que importa: cuál de las tres formas resuelve en el runtime real. La que responda
     OK es la que hay que escribir en TODOS los imports relativos de las funciones. */
  for (const spec of ['./_guard.ts', './_guard.js', './_guard']) {
    try {
      await import(spec)
      info['import ' + spec] = 'OK'
    } catch (e) {
      info['import ' + spec] = (e as Error).message.split('
')[0]
    }
  }
