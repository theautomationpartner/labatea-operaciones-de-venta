...
function mapProformaProducto(sub: MondayItem): PresupuestoProducto {
  ...
  return {
    ...
    // Descuento manual del producto tal como quedó guardado en la proforma.
...
export async function getProformasCliente(clienteItemId: string): Promise<ProformaVigente[]> {
  ...
  const data = await mondayApi<{ items: (MondayItem & { subitems: MondayItem[] })[] }>(
    `query ($ids: [ID!]) {
      items(ids: $ids) {
        id name
        column_values(ids: ["${COL.proforma.importe}","${COL.proforma.rentabilidad}","${COL.proforma.total}"]) {
          id text
          ... on MirrorValue { display_value }
        }
        subitems {
          id name
          column_values(ids: ["${COL.proformaSub.producto}","${COL.proformaSub.unidadMedida}","${COL.proformaSub.cantidad}","${COL.proformaSub.precioUnit}","${COL.proformaSub.descuento}","${COL.proformaSub.impBonificado}","${COL.proformaSub.iva}","${COL.proformaSub.total}","${COL.proformaSub.rentabilidad}","${COL.proformaSub.subtotal}"]) {
            id text
            ... on MirrorValue { display_value }
...
export async function crearProforma(datos: DatosProforma): Promise<ProformaCreada> {
  ...
  const filas = lineas.map((l) => {
    ...
    const ivaLinea = round2((totalLinea * (l.iva ?? IVA_DEFECTO_PROFORMA)) / 100)
    return { l, bonifUnit, totalLinea, ivaLinea }
  })
  ...
  for (const { l, bonifUnit, totalLinea, ivaLinea } of filas) {
    const cv: Record<string, unknown> = {
      ...
      [COL.proformaSub.rentabilidad]: Math.round(l.rentabilidad),
    }
