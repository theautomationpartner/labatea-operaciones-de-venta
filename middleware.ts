 ...
 * Monday usando el token del servidor (`MONDAY_TOKEN`), que es lo único que un atacante querría
 * usar: con esa ruta abierta, cualquiera escribe en los tableros sin tener un token propio.
 *
 ...
 * verificación del `sessionToken` de Monday y el código de 6 dígitos); un cliente que no sea un
 * navegador puede inventar el `Referer`. Sirve contra el acceso casual desde el navegador, contra
 * páginas ajenas que embeban la app y contra scripts que peguen a la ruta desde otro sitio.
 *
